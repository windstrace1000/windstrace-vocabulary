...
export default function SearchTab({
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2">
      ...
      <div className="bg-white p-4 sm:p-6 rounded-2xl shadow-sm border border-slate-100">
        ...
        <form onSubmit={handleSubmit} className="flex flex-nowrap items-center gap-2">
          ...
          >
            ...
            <span className="text-sm sm:text-base">{isSearching ? (window.innerWidth < 640 ? '...' : (searchQuery && /[\u4e00-\u9fa5]/.test(searchQuery) ? '翻譯中...' : '查詢中...')) : '查詢'}</span>
          </button>
      ...
      {/* 查詢結果 */}
      {(searchResult || searchCandidates?.length > 0) && (
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden relative">
          {/* 載入中覆蓋層 (切換單字時顯示，但不讓卡片消失) */}
          {isSearching && searchResult && (
            <div className="absolute inset-0 bg-white/60 backdrop-blur-[1px] z-10 flex items-center justify-center animate-in fade-in duration-300">
              <div className="flex flex-col items-center gap-3">
                ...
                <span className="text-sm font-medium text-slate-500">正在切換單字內容...</span>
              </div>
            </div>
          )}

          {/* 候選單字切換 (持久顯示，直到新的中文查詢發生) */}
          {searchCandidates?.length > 1 && (
            <div className="px-4 py-3 bg-slate-50 border-b border-slate-100 flex flex-wrap items-center gap-2">
              <span className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-tight">您可能想找：</span>
              {searchCandidates.map((c, idx) => (
                <button
                  key={idx}
                  onClick={() => onSearch(c.word, { isCandidateSelection: true })}
                  disabled={searchResult && c.word.toLowerCase() === searchResult.word.toLowerCase()}
                  className={`px-2.5 py-1 rounded-full text-xs font-medium transition-all ${
                    searchResult && c.word.toLowerCase() === searchResult.word.toLowerCase()
                      ? 'bg-indigo-600 text-white shadow-sm'
                      : 'bg-white text-slate-600 border border-slate-200 hover:border-indigo-300 hover:text-indigo-600 disabled:opacity-80'
                  }`}
                >
                  {c.word} <span className="opacity-70 text-[10px] ml-0.5">({c.translation})</span>
                </button>
              ))}
            </div>
          )}
                            ...
                            <tr key={idx} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/50">
                              ...
                              <td className="py-2.5 px-3 sm:px-4 font-medium text-indigo-700">
                                <div className="flex items-center justify-between gap-1">
                                  <button onClick={() => onSearch(form.formWord, { isCandidateSelection: true })} className="hover:underline hover:text-indigo-800 text-left transition-colors truncate">{form.formWord}</button>
                        ...
                        <tbody>
                          {searchResult.similarWords.map((sim, idx) => (
                            <tr key={idx} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/50">
                              <td className="py-2.5 px-3 sm:px-4 font-medium text-indigo-700">
                                <div className="flex items-center justify-between gap-1">
                                  <button onClick={() => onSearch(sim.word, { isCandidateSelection: true })} className="hover:underline hover:text-indigo-800 text-left transition-colors truncate">{sim.word}</button>
                        ...
                        <tbody>
                          {searchResult.relatedPhrases.map((item, idx) => (
                            <tr key={idx} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/50">
                              <td className="py-2.5 px-3 sm:px-4 font-medium text-indigo-700">
                                <div className="flex items-center justify-between gap-1">
                                  <button onClick={() => onSearch(item.phrase, { isCandidateSelection: true })} className="hover:underline hover:text-indigo-800 text-left transition-colors break-words">{item.phrase}</button>
